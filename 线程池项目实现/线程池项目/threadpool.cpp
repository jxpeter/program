...
ThreadPool::~ThreadPool()
{
	isPooolRunning_ = false;
	//等待线程池里面的线程返回，有两种状态，阻塞&正在执行
...
Result ThreadPoll::submitTask(std::shared_ptr<Task> sp)
{
	//获取锁
	std::unique_lock<std::mutex> lock(taskQueMtx_);
...
bool ThreadPoll::checkRunningState() const
{
...
void Thread::start()
{
	...
	t.detach(); //设置分离线程 pthread-detach  pthread_t  设置成分离线程
}
